<template>
  <h2>{{ presentar }}</h2>
  <p>{{ numero * numero }}</p>
  <p>{{ numero }}<sup>2</sup>={{ numero * numero }}</p>
...
<script>
export default {
  props: ["titulo", "num"], //forma basica de declarar un props
  props: {
    titulo: String,
    num: {
      type: Number,
      ...
      default: 10,
      validator(value) {
        //toda la programación retorna un valor boleano
        return value > 0;
      },
    },
  },
  ...
  methods: {
    ...
    calcularCuadrado2(num1, num2) {
      console.log("ingreso al metodo 2");
      return num1 * num2;
    },
    incrementar() {
      this.numero++;
